...
module.exports = async orderProducts => {
  try {
    const productsAvailibilityDetails = await orderProducts.reduce(async (resultPromise, orderItem) => {
    ...
    return {
      productsAvailibilityStatus: productsAvailibilityDetails.every(product => product.available),
